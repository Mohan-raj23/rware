...
class Warehouse(gym.Env):
    ...
    def step(
        ...
        committed_agents = set([self.agents[id_ - 1] for id_ in committed_agents])
        failed_agents = set(self.agents) - committed_agents
        ...
        # Add reward based on proximity to goal
        for agent in self.agents:
            for goal in self.goals:
                distance_to_goal = abs(agent.x - goal[0]) + abs(agent.y - goal[1])
                rewards[agent.id - 1] += 0.1 / (distance_to_goal + 1)

            # Encourage movement by rewarding non-zero movement
            if agent.prev_x != agent.x or agent.prev_y != agent.y:
